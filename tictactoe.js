...
function Board (options){
  ...
  function render(){
    ...
    function htmlQ1(){
      return `<div id="view1"><h3>Which would you prefer today  ?\n</h3>
      ${buttonHTML(1, "1player", "human vs computer(hard)")}
      ${buttonHTML(1, "2players", "human vs human")}
      </div>`
    }

    function htmlQ2(){
      const html2=`<div id="view2"><h3>${!state.players[1].isComputer? "Player 1, <br />" : ""}Which symbol do you choose ?</h3>
      ${buttonHTML(2, "X", "X")}
      ${buttonHTML(2, "O", "O")}`
      return html2
    }
    ...
    function htmlGameEnd (){
      ...
      if(result !== RESULT.tie)
        resultText = getPlayerName(result) + " WINS!!"

      let htmlBefore = `<h3>${resultText} ${htmlSpaces(2)} Click here to restart the game</h3> `
      let board = state.game._gameBoard.reduce(function(acc,curr,rowIndex){
